...
plot.segementation.areas <- 0 # depreciated. keep at 0 or the code crashes since there is no Area column 
...
#  source("Advanced spatial 2 - modified functions.R") 
  # watch out: the way this is scripted you would load the default functions the first time you run through STAGE 0 simply because we source the adapted functions before,
  # and then the code loads spectre. Because of that, the source line comes a second time in STAGE 0 after we loaded spectre. Just to be sure...
...
# Load Metadata file from absolute path to be sure we are loading always the correct meta:
        library(readxl)
        # the original cols were:   ROI,  Sample , Group , Batch
        # the personal cols are  Ablation_ID	ROI_ID	Diet	Sample_week	CD45_gate	INS_gate	Tissue  Batch
        setwd(PrimaryDirectory)
    ...
    ### Set InputDirectory 
        ...
        all.tsv.files <- list.files(pattern = "\\.tsv$") # this ensures that only .txt is listed, and not .ttxt or so

        
        
        
        # watch out. this is only working with one file in there yet.
        PCFdata <-readr::read_tsv(all.tsv.files[1])
        Xeniumdata <-readr::read_tsv(all.tsv.files[2])
        

        PCFdata <- tibble::add_column(PCFdata, "tsv file" = all.tsv.files[1], .after = 2)
        
        
        
        # fix the PCFdata:
        # for the next command, load tidy first:
    
        
        PCFdata<-PCFdata %>% dplyr::rename_all(~ make.names(.) )
        names(PCFdata)
        
        
        # depending on your QuPath processing, you might have some columns that contains all NA, such as
        # Name and/or Classification.
        # so search in PCFdata acroos all columns for those that are all NA and remove them:
        PCFdata <- PCFdata[ , !names(PCFdata) %in% names(PCFdata)[sapply(PCFdata, function(x) all(is.na(x)))]  ]
        
        
        # cell Id in PCF is called  Object.ID
        # Xeniums first column does not carry that name.
        # rename first column of Xeniumdata to be called Object.ID
        names(Xeniumdata)[1] <- "Object.ID"
        
        Xenium.data.colnames <- names(Xeniumdata)
        
        
        # brute force the Xenium columns into PCF:
        all.cells <- do.add.cols(as.data.frame(PCFdata), base.col = 'Object.ID', add.dat = as.data.frame(Xeniumdata), add.by = 'Object.ID')
        ...
        cellular.cols <- names(PCFdata %>% dplyr:: select(starts_with( paste0(Markers2Process, qupath.separator,cellular.segment.readout) )))
        ...
        if(length(Markers2Process) != length(cellular.cols) ){
          ...
          temp.string <- names(PCFdata %>% 
                                 dplyr:: select(-starts_with( paste0(Markers2Process, qupath.separator,cellular.segment.readout) )) %>% 
                                 ...
                                 dplyr:: select(-starts_with( c("Image","Object.ID","Object.type", "Name", "Classification" , 
                                                                "TMA.core", "Parent" , "ROI" , "Centroid.X.µm","Centroid.Y.µm","tsv.file") )) %>%
    ...
    ### Create output directory
       ...
       setwd(OutputDirectory)
        ...
        ### Pulling cellular data into a separate data.table is depreciated. We do that in the first script and source cell.dat right away
      #  cell.dat <- do.pull.data(spatial.dat, 'CellData')
      #  cell.dat
      #  message("\nNew cellular data file (cell.dat) created\n\n")
        ### Add sample metadata
        # so the cell.dat comes already with ROI as col name.
        # before we drop the sample.meta in, lets rename that:
        

        
###### Drop bad ROIs from cell.dat ######
     
        # We take out these ROIs that are too small or that the segmentation was bad:
      #  cell.dat <- cell.dat[ grep("IM_20220618_OT4_s0_p4_r6_a6_ac", cell.dat$ROI, invert = TRUE) , ]
      #  cell.dat <- cell.dat[ grep("IM_20220618_OT4_s0_p4_r7_a7_ac", cell.dat$ROI, invert = TRUE) , ]
      #  cell.dat <- cell.dat[ grep("IM_20220618_OT4_s0_p4_r8_a8_ac", cell.dat$ROI, invert = TRUE) , ]
      #  cell.dat <- cell.dat[ grep("IM_20220618_OT4_s0_p4_r9_a9_ac", cell.dat$ROI, invert = TRUE) , ]       
      #  cell.dat <- cell.dat[ grep("IM_20220618_OT4_s0_p4_r10_a10_ac", cell.dat$ROI, invert = TRUE) , ]    
        
       
       

       
       # again, watch out! mapping along cores is not possible anymore if we load more than one
       # qptiff from more than one tsv.file...

       # before we map metadata, we need to make sure the metadata column is there and in the proper format - and unique:
       
       # so the code will take Image and Core columns and paste them, that will be unique for any data we feed in:
       # paste TMA.ID and TMA.core into a new column called TMA.ID_core in sample.meta
       sample.meta$TMA.ID_core <- paste(sample.meta$TMA.ID,sample.meta$TMA.core, sep="_")
  
       
       # and the same for the PCFdata:
       PCFdata$TMA.ID_core <- paste(PCFdata$Image,PCFdata$TMA.core, sep="_")
       ...
       if( "TMA.core" %in% names(sample.meta) ){
         ...
         if( all(  grepl("^[A-Za-z]-[0-9]$", sample.meta$TMA.core)) ){
           ...
           sample.meta <- sample.meta %>% select(-TMA.core)
           
           #ready to map meta into df
           PCFdata <- do.add.cols(as.data.frame(PCFdata), base.col = 'TMA.ID_core', add.dat = sample.meta, add.by = 'TMA.ID_core')
           message("Mapping metadata to the cellular data complete")
         }else{
          
     # lets attempt to fix it, maybe the column is in A1 format:
             sample.meta$TMA.core <-  gsub("([A-Za-z]+)([0-9]+)", "\\1-\\2", sample.meta$TMA.core)
             
             # re-create the proper column:
             sample.meta$TMA.ID_core <- paste(sample.meta$TMA.ID,sample.meta$TMA.core, sep="_")
         ...
         message("Mapping metadata failed. Make sure your TMA.ID column in metadata matches the image ID of the tsv")
         stop()
       }
        ...
        # now that we mapped batch numbers, we can clear whole ROIs:
        
        # If an entire TMA needs to be dropped, this can be done now that
        #  we mapped batch numbers and TMA IDs from the metadata. What an expensive waste the next line creates...
        #cell.dat <- cell.dat[ grep("OT31",  cell.dat$Batch, invert = TRUE) , ]
        
       
        
       
       # we will now switch to cell.dat nomenclature and store away the unprocessed PCFdata away:
       # let us store away the cleared cell.dat
       cell.dat <- PCFdata
       ...
       all.TMAs  <- unique(cell.dat$Image)
       amount.of.TMAs  <- length(all.TMAs)
        ...
        if(transform.rawdata==T){
          markercomparision <-lapply(scatter.plots, function(x) ifelse(is.na(x), NA, paste0(x, qupath.separator,cellular.segment.readout)  ))
...
names(Batches_palette) <- all.TMAs  
...
# the trick is that quantiles_table_long was rbound running through "batch in all.TMAs". 
# therefore, if we pull out temp.quantile.values in the upcoming loop, these values have the same order as all.TMAs
# so there is no need to cbind the batches everytime we build up the scaling.factor dataframe: the scaling factors follow the same order as in all.TMAs:
scaling.factors <- data.frame( Batch.ID = all.TMAs   )
...
# lets run through the markers again and calculate the SF, given the percentile we find in the matrix (or rather, the batchnormalize.percentile vector derived from that matrix )
 ...
 # temp.quantile.values <- deframe( subset(quantiles_tibble_long, Percentile==batchnormalize.percentile[m]  )[,batchnormalize.marker.transformed[m]]   )
  ...
  temp.SF.values <- data.frame(
    ...
    Batch = subset(quantiles_table_long, Percentile %in% batchnormalize.percentile[m]  )$Batch, # this is equivalent with all.TMAs, but I wanna be sure the order is the same
...
#      )
 ...
 # names(quantiles.list) <- all.TMAs 
  ...
  quantiles.df.long <- cbind(quantiles.df.long,  rep(all.TMAs, each= length(batchnormalize.marker) ) )
...
all.TMAs <- sort(unique(gated.cell.dat$Batch)) 
amount.of.TMAs <- length(all.TMAs)
...
################# Flag cell segmentation penalty #################

      

# since v2 the code does not subset cell.dat, but will simply flag the cell as 1) gate outlier and 2) segmentation area outlier:
# lets create outlier flag columns. 0 means active datapoint, 1 means outlier
# if you change the gate settings, this brute force here will simply overwrite whatever was in the cell.dat and makes sure you stay up-to-date no matter what:
      
# for the area we need a range penalty:
  # it uses j <= i & i <= k  to flag "in range, 0"
  #cell.dat$outside.area <-mapply(function.range.penalty,i=cell.dat$Area, j=cell.dat$area.highpass, k=cell.dat$area.lowpass  )
...
for(g in threshold.gates){
 ...
 }# end protect from gating if there are no Hgate or Lgate columns present
...
# watch out, I will make this automated at some point since this is the way to go. but for now there is a good amount of manual adaptation here:

# if you want a multi-semented line as Hgate, you dont supply them via metadata file, but here:
# be aware that in stage, this wont set flag columns yet, it will just print it and then use the line to subdivide a gated subset accordingly.
# these are the points with CD45_Er168 on x, GLP.1R_Yb174 on y
# the line runs from "bottom up" along y:



#range(cell.dat$CD45_Sm152)
#range(cell.dat$CD3_Er170)

polygon.gates <- list(
#do.not.use.polygon.gates <- list(
  
  #> range(cell.dat$CD45..Mean)
  #[1]  0.000 7371.741
  #> range(cell.dat$CD20..Mean)
  #[1]  27.4875 3871.8350
  ...
  "CD20.highpass"  = tribble(
    #x            #y   
    ~CD45..Mean, ~CD20..Mean,
    -1,  4000 , # <- if you intend to fine-tune that graph, make sure that the scaling does not bring this corner point inside the data range!!
    300, 900,
    6000,400,
    7400,  25
    # <- if you intend to fine-tune that graph, make sure that the scaling does not bring this corner point inside the data range!!
  ),
  
  
  #> range(cell.dat$CD45..Mean)
  #[1]  0.000 7371.741
  #> range(cell.dat$CD3e..Mean)
  #[1]  0.4274 32871.6016
  
  "CD3e.highpass"  = tribble(
    #x            #y   
    ~CD45..Mean, ~CD3e..Mean,
    -1,  35000 , # <- if you intend to fine-tune that graph, make sure that the scaling does not bring this corner point inside the data range!!
   30,5000,
200,3000,
1000,2100,
3000,2000,
...
#> range(cell.dat$CD45..Mean)
#[1]  0.000 7371.741
#> range(cell.dat$E.cadherin..Mean)
#[1]  0.00 14538.43

"CD45.highpass"  = tribble(
  #x            #y   
  ~CD45..Mean, ~E.cadherin..Mean,
  -1,  40000 , # <- if you intend to fine-tune that graph, make sure that the scaling does not bring this corner point inside the data range!!
60,20000,
300,2000,
450,500,
500,20,
...
#  c("CD45_Sm152", "CD19_Nd142"  , NA), # <- this is to see gating into G1
#c("CD45_Sm152", "CD3_Er170", NA)#

# now this shape might give a good first fit but you might wanna fine-tune the position even after batch-normalization:
# you need to write this dataframe either way since the flagging will need it. Fill it with 1 if you dont want to fine-tune gates on any TMA
#finetune.CD45segmentline <- data.frame( Batch.ID = all.TMAs   )
#finetune.CD45segmentline[, paste0( "CD45_Sm152" )] <- 1
#finetune.CD45segmentline[, paste0( "CD3_Er170" )] <- 1


# you do it like this:
#finetune.CD45segmentline[  match ("OT11", scaling.factors$Batch.ID), "CD45_Er168"]  <- 0.6
#finetune.CD45segmentline[  match ("OT11", scaling.factors$Batch.ID), "GLP.1R_Yb174"]  <- 0.9 


# new code from Laias data:
...
if(exists("polygon.gates") ){
  ...
  finetune.segmentline <- data.frame( Batch.ID = all.TMAs   )
  finetune.segmentline[, all.polygon.gate.markers ] <- 1
  
  
  
  
  # flagging via polygons happens automatically:
 # this is a bit hard to read. so lets spill it out:
  
  # we will create multisegmented lines given the xy points in the tibbles
  # since the script does not know on which axis the thresholding marker is drawn, we need to first grepl the thresholding marker from either the x or x axis of the tibble.
  # then, depending on which axis it lays, we will either check if a datapoint lays right hand of the line (x is the dependent variable, the threshold is on x and you need unique y values in the tibble
  # or the threshold is on the y axis and you need unique x numbers in the tibble
  # how we push the coordinates into multisegmentGATEline is chosen automatically. after f is defined, we flag the data, and we correct the line TMA-wise, meaning we move it by factors up/down/left/right
...
#   }
  ...
  temp.gated.marker <-  paste0(  sub(".[^.]+$", "", g )  , qupath.separator,cellular.segment.readout ) 
  # pull all cols with that AB. same game as before: paste0 the underscore before grepping pulls only one marker:
 # temp.gated.marker <- names(cell.dat)[ grep( paste0( temp.gated.marker  , qupath.separator,cellular.segment.readout )  , colnames(cell.dat))]
...
if(run.experimental.code==1){
  ...
  # Birigt prefers a nicely sitting CD4 gate on the ROIs, so lets check that for subset 2:
  
ggplot( data=cell.dat ,
          aes( x= CD45..Mean,   y= CD4..Mean , color = Ki67..Mean ) ) +  #   as.factor(outside.CD20.highpass)
...
ggsave("CD20_gate.jpg", g, width = 20, height = 16, units = "cm", dpi = 300)
  ...
  percentile <- as.numeric(quantile( gated.cell.dat$CD68..Mean, # we compute the quantiles from non-ctr data
                                     probs = c( 0.0 , 0.75, 0.95))) # ,na.rm=TRUE can cause some weird behaviour of limits
  lower.threshold <- percentile[1]
  median <- percentile[2]
  upper.threshold <- percentile[3]
  
g<-ggplot( data=cell.dat ,
          aes( x= CD45..Mean,   y= CD3e..Mean , color = CD68..Mean ) ) + # color =CD68..Mean as.factor(outside.CD3e.highpass)
...
#    )+  
  ...
  coord_cartesian(ylim=c(as.numeric(quantile(cell.dat[[yMarker]], 0.001)),as.numeric(quantile(cell.dat[[yMarker]], 0.9995))), 
                  xlim=c(as.numeric(quantile(cell.dat[[xMarker]], 0.001)),as.numeric(quantile(cell.dat[[xMarker]], 0.9995)))
                  )+
...
ggsave("CD3_gate_(withMacsShown).jpg", g, width = 20, height = 16, units = "cm", dpi = 300)
...
if(plot.TMAwise.gates == 1){
  ...
  if( identical(all.gate.flags, character(0)) ){  
    ...
    pb <- progress_bar$new(format = "[:bar] :percent [Plotting marker scatter | :eta]",
                           total = length(markercomparision)*length(all.TMAs) , 
                           show_after=0, #allows to call it right way 
                           current = "|",    # Current bar character
    ...
    pb <- progress_bar$new(format = "[:bar] :percent [Plotting marker scatter | :eta]",
                           total =length(markercomparision)*length(all.TMAs)*length(all.gate.flags)*2 , # *2 cause we run over the tickmark of the first condion in this if-else
                           show_after=0, #allows to call it right way 
                           current = "|",    # Current bar character
...
# using the new plotting design: the old can be found in v14

g <- 
  ggplot(data=cell.dat, aes(x=Image, y= Area,color=as.factor(outside.area) )) + #, colour=Batch 
...
filename <- "QC_SegementationAreas.png" 
ggsave(filename,g, width = 30, height = 17, units = "cm",dpi = 1200)
  ...
  for(i in all.TMAs ){
   ...
   t[[TMAlist.pos]] <- 
      plot.scatter.density.v2(indata=temp.TMAwise.OTdata, 
                                               ...
                                               passed.gate.color = "#308441",
                                               rejected.gate.color = "#F6CD7A",   # default "#EB8563"
                                              #  titlestring=paste0("Subset in: ", GATEsubsetNames[s] ) # title string defaults to i as all.TMAs object
    )
...
all.TMAs <- sort(unique(gated.cell.dat$Image)) 
amount.of.TMAs <- length(all.TMAs)
...
for(m in cellular.cols ){
  ...
  g <- 
    ...
    # scico::scale_fill_scico(palette = "roma", direction=-1)+ # bilbao
    #scale_color_manual(values = scico(length(all.TMAs), begin = 0.12, palette = "roma"))+
    #scale_fill_manual(values = scico(length(all.TMAs), begin = 0.12, palette = "roma"))+
